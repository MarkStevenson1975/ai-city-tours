...
export interface StopInput {
  ...
  /** Optional ordered via-points (max 30) on the way to the next stop, so the
   *  route follows a published path rather than the shortest line. */
  via_points?: { lat: number; lng: number }[];
}

/** Keep only well-formed points, capped at 30; null when there are none. */
...
export async function updateStop(input: UpdateStopInput) {
  ...
  const { error } = await admin
    ...
    .update({
      ...
      via_points: cleanViaPoints(input.via_points),
    })
    .eq('id', input.stopId);
